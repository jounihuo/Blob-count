...
def _snap():
    ...
    rows,cols = camera_capture.shape
    M = cv2.getRotationMatrix2D((cols/2,rows/2),180,1)
    camera_capture = cv2.warpAffine(camera_capture,M,(cols,rows))
	#Saving the image
    file = "snap.jpg"
    ...
    image = cv2.imread('snap.jpg')
    ax.imshow(image)
	#Inverse the grayscale image if needed
...
w = Tk.Scale(master=root, from_=1, to=100, label='Sensitivity')
w.pack(side=Tk.BOTTOM)
w.set(50)
w2 = Tk.Scale(master=root, from_=1, to=50, label='Size')
